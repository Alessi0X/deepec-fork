...
# Compatibility layer for loading old scikit-learn pickle files
class CompatibilityUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        # Remap old sklearn module paths to new ones
        if module == "sklearn.preprocessing.label":
            module = "sklearn.preprocessing._label"
        return super().find_class(module, name)


def load_pickle_compat(file_path):
    """Load pickle file with compatibility for old scikit-learn versions"""
    ...
    for seq_record in SeqIO.parse(input_handle, "fasta"):
        ...
        if len(seq) <= 1000:
            ...
            for i in range(0, len(seq) - l + 1, 100):
                new_seq_id = "%s_SEPARATED_SEQUENCE_(%s_%s)" % (
                    seq_id,
                    i + 1,
                    i + l + 1,
                )
    ...
    for seq_record in SeqIO.parse(input_handle, "fasta"):
        try:
            ...
            if len(seq) >= 10 and len(seq) <= 1000:
                ...
                flatten_encoded_vector_str = [
                    str(each_val) for each_val in flatten_encoded_vector
                ]
...
def predict_target_ec(df, output_file, DeepEC_model):
    ...
    model = load_model(DeepEC_model, compile=False)
...
def predict_ec(df, output_file, DeepEC_model, MultiLabelBinarizer, threshold=0.5):
    ...
    lb = load_pickle_compat(MultiLabelBinarizer)
